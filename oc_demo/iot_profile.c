...
#define CN_PROFILE_TOPICFMT_CMDRESP    "mqtt_huaweicar_demo/%s/sys/commands/response/request_id=%s"
...
#define CN_PROFILE_TOPICFMT_PROPERTYREPORT    "mqtt_huaweicar_demo/%s/led_report"
...
int SimplePropertyReport(const char *deviceID, const char *key, const char *value)
{
    char *topic;
    char msg[128];
    ...
    // 发送 MQTT 消息
    int ret = IotSendMsg(0, topic, msg);
