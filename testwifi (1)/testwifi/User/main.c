...
// 全局变量
char topic_id[100] = "mqtt_stm32_demo/testUser4002/test";
char temp_topic_id[100] = "mqtt_stm32_demo/testUser4002/temp_report";
char humi_topic_id[100] = "mqtt_stm32_demo/testUser4002/humi_report";
...
void data_pros()	//数据处理函数
{
   ...
   // publish data to MQTT topic
    if (ESP8266_MQTT_Public_key_Value(temp_topic_id, "temperature", temp_buf)) {
        printf("Published temperature successfully!\r\n");
    } else {
        printf("Failed to publish temperature!\r\n");
    }
    
    if (ESP8266_MQTT_Public_key_Value(humi_topic_id, "humidity", humi_buf)) {
